"""Tests for Paxy Courier FastAPI endpoints."""
...
VALID_CREDENTIALS = {"api_key": "test-key", "api_token": "test-token"}
VALID_COMMAND = {
    "service_name": "STANDARD",
    "shipment_date": "2026-03-01",
    "content": "Electronics",
    "parcels": [{"weight": 2.5, "length": 30, "width": 20, "height": 15}],
...
def test_create_shipment_success(mock_integration):
    ...
    data = response.json()
    assert data["order_id"] == "PAX-001"
    assert data["waybill_number"] == "PAX123456"
...
@pytest.mark.parametrize(
    "status_value",
    ["CREATED", "PICKED_UP", "IN_TRANSIT", "DELIVERED", "RETURNED"],
)
@patch("src.app.integration")
