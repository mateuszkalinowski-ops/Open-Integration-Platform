"""SkanujFakture Integrator — FastAPI application entry point."""
...
async def lifespan(application: FastAPI):  # type: ignore[no-untyped-def]
    setup_logging(f"skanuj-fakture-integrator-v{settings.app_version}", settings.log_level)
    logger.info("Starting SkanujFakture Integrator v%s", settings.app_version)
    ...
    integration = SkanujFaktureIntegration(account_manager)
    ...
    if settings.polling_enabled:
        poller = DocumentPoller(account_manager, state_store, kafka_producer)
        app_state.poller = poller
        poller_task = asyncio.create_task(poller.start())
        logger.info("Document poller scheduled every %ds", settings.polling_interval_seconds)

    logger.info(
        "SkanujFakture Integrator ready — accounts=%d, kafka=%s, polling=%s",
    ...
    logger.info("Shutting down SkanujFakture Integrator")
    if app_state.poller:
