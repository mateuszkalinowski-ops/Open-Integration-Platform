"""IdoSell Integrator — FastAPI application entry point."""
...
async def lifespan(application: FastAPI):  # type: ignore[no-untyped-def]
    setup_logging(f"idosell-integrator-v{settings.app_version}", settings.log_level)
    logger.info("Starting IdoSell Integrator v%s", settings.app_version)

    state_store = StateStore()
    await state_store.initialize()
    app_state.state_store = state_store

    auth_manager = IdoSellAuthManager()

    client = IdoSellClient(auth_manager)
    ...
    integration = IdoSellIntegration(client, account_manager)
    ...
    if settings.scraping_enabled:
        ...
        logger.info("IdoSell scraper scheduled every %ds", settings.scraping_interval_seconds)

    logger.info(
        "IdoSell Integrator ready — accounts=%d, kafka=%s, scraping=%s",
    ...
    logger.info("Shutting down IdoSell Integrator")
