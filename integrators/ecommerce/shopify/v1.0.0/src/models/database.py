...
Stores access tokens (encrypted) and scraper checkpoint state.
"""
...
class TokenStore:
    """Async SQLite-backed store for access tokens and scraper state."""
    ...
    async def save_token(self, account_name: str, token_data: dict[str, Any]) -> None:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute(
                """INSERT INTO access_tokens (account_name, token_data)
                   VALUES (?, ?)
    ...
    async def load_all_tokens(self) -> dict[str, dict[str, Any]]:
        ...
        async with aiosqlite.connect(self._db_path) as db:
            async with db.execute("SELECT account_name, token_data FROM access_tokens") as cursor:
    ...
    async def delete_token(self, account_name: str) -> None:
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute("DELETE FROM access_tokens WHERE account_name = ?", (account_name,))
            await db.commit()

    async def save_last_order_id(self, account_name: str, order_id: str) -> None:
        async with aiosqlite.connect(self._db_path) as db:
            await db.execute(
                """INSERT INTO scraper_state (account_name, last_order_id)
                   VALUES (?, ?)
                   ON CONFLICT(account_name) DO UPDATE SET last_order_id=excluded.last_order_id, updated_at=CURRENT_TIMESTAMP""",
                (account_name, order_id),
            )
            await db.commit()

    async def load_all_last_order_ids(self) -> dict[str, str]:
        result: dict[str, str] = {}
        async with aiosqlite.connect(self._db_path) as db:
            async with db.execute("SELECT account_name, last_order_id FROM scraper_state") as cursor:
                async for row in cursor:
                    result[row[0]] = row[1]
        return result
