"""Amazon Integrator — FastAPI application entry point."""
...
async def lifespan(application: FastAPI):  # type: ignore[no-untyped-def]
    setup_logging(f"amazon-integrator-v{settings.app_version}", settings.log_level)
    logger.info("Starting Amazon Integrator v%s", settings.app_version)
    ...
    integration = AmazonIntegration(client, account_manager)
    ...
    if settings.scraping_enabled:
        scraper = AmazonScraper(client, account_manager, state_store, kafka_producer)
        app_state.scraper = scraper
        scraper_task = asyncio.create_task(scraper.start())
        logger.info("Amazon scraper scheduled every %ds", settings.scraping_interval_seconds)

    logger.info(
        "Amazon Integrator ready — accounts=%d, kafka=%s, scraping=%s",
    ...
    logger.info("Shutting down Amazon Integrator")
...
def create_app() -> FastAPI:
    application = FastAPI(
        title="Pinquark Amazon Integrator",
        description="E-commerce integration between Pinquark platform and Amazon Selling Partner API",
