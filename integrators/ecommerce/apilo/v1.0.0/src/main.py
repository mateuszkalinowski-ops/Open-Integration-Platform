"""Apilo Integrator — FastAPI application entry point."""
...
async def lifespan(application: FastAPI):  # type: ignore[no-untyped-def]
    setup_logging(f"apilo-integrator-v{settings.app_version}", settings.log_level)
    logger.info("Starting Apilo Integrator v%s", settings.app_version)
    ...
    integration = ApiloIntegration(client, account_manager)
    ...
    if settings.scraping_enabled:
        scraper = ApiloScraper(client, account_manager, state_store, kafka_producer)
        app_state.scraper = scraper
        scraper_task = asyncio.create_task(scraper.start())
        logger.info("Apilo scraper scheduled every %ds", settings.scraping_interval_seconds)

    logger.info(
        "Apilo Integrator ready — accounts=%d, kafka=%s, scraping=%s",
    ...
    logger.info("Shutting down Apilo Integrator")
