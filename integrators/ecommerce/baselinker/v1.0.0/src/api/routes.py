"""FastAPI routes for BaseLinker integrator."""
...
async def list_accounts() -> list[dict[str, str]]:
    ...
    return [{"name": a.name, "environment": a.environment} for a in accounts]


@router.post("/accounts", status_code=201)
async def add_account(req: AccountCreateRequest) -> dict[str, str]:
    account = BaseLinkerAccountConfig(**req.model_dump())
...
async def list_orders(
    account_name: str = Query(..., description="BaseLinker account name"),
    since: datetime | None = Query(None, description="Fetch orders confirmed since this timestamp"),
